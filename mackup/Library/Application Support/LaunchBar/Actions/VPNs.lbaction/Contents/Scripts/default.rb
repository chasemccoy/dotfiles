...
def vpns_with_status(status)
    output = `/usr/sbin/scutil --nc list | /usr/bin/grep -e "^\* (#{status})" | /usr/bin/awk --re-interval '{match($0, /".*"/ ); print substr($0, RSTART+1, RLENGTH-2); }'`
...
def type_of_vpn(vpn_name)
    `/usr/sbin/scutil --nc show "#{vpn_name}" | /usr/bin/head -1 | /usr/bin/awk --re-interval '{match($0, /\\[.*\\]$/ ); print substr($0, RSTART+1, RLENGTH-2); }'`
end
